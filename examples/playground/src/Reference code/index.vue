  ...
  <div class="math-editor">
    ...
    <div class="toolbar">
      ...
      <div class="format-group">
        <button @click="toggleFormat('bold')" :class="{ active: isFormatActive('bold') }" title="Bold">
          <strong>B</strong>
        </button>
        <button @click="toggleFormat('italic')" :class="{ active: isFormatActive('italic') }" title="Italic">
          <em>I</em>
        </button>
        <button @click="toggleFormat('underline')" :class="{ active: isFormatActive('underline') }" title="Underline">
          <u>U</u>
        </button>
        <button @click="toggleFormat('strikethrough')" :class="{ active: isFormatActive('strikethrough') }" title="Strikethrough">
          <s>S</s>
        </button>
      </div>
      ...
      <div class="math-group">
        <button @click="() => { ensureCursorInEditor(); showFormulaEditor(); }" class="formula-btn" title="Insert Math Formula">
      ...
      <div class="image-group">
        ...
        />
        <button @click="triggerImageUpload" class="image-btn" title="Insert Image" :disabled="uploadLoading" :loading="uploadLoading">
...
const convertFromStandardSyntax = async (content: string): Promise<string> => {
  ...
  htmlContent = htmlContent
    ...
    .replace(/~~(.*?)~~/g, '<s>$1</s>') // 删除线
    .replace(/!\[([^\]]*)\]\(([^)]*)\)/g, '<img src="$2" alt="$1" style="max-width: 100%; height: auto; margin: 8px 0; border-radius: 4px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">') // Markdown格式图片（向后兼容）
    .replace(/\n/g, '<br>') // 换行
...
const updateExistingFormula = (latex: string) => {
  ...
  if (editingFormulaElement.value.tagName.toLowerCase() === 'svg') {
    ...
    editingFormulaElement.value.parentNode?.replaceChild(newFormulaElement, editingFormulaElement.value)
  }
}
...
const renderLatex = (latex: string) => {
  return latex
    ...
    .replace(/\\sqrt\{([^}]+)\}/g, '<span class="sqrt-wrapper"><span class="sqrt-radical">√</span><span class="sqrt-content">$1</span></span>')
    .replace(/\^2/g, '²')
    .replace(/\^3/g, '³')
    .replace(/\^(\{[^}]+\})/g, '<sup>$1</sup>')
...
const ensureCursorInEditor = () => {
  ...
  if (selection.rangeCount === 0) {
    ...
    while (currentNode && currentNode !== document.body) {
      ...
      if ((currentNode as Element).classList?.contains('toolbar') || 
...
.clear-all-btn {
  background: linear-gradient(135deg,rgba(255, 0, 0, 0.1) 0%, rgba(255, 0, 0, 0.1) 100%) !important;
...
.clear-all-btn:hover {
  background: linear-gradient(135deg, rgba(255, 0, 0, 0.2) 0%, rgba(255, 0, 0, 0.2) 100%) !important;
